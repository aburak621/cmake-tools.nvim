...
require("cmake-tools").setup {
  ...
  cmake_terminal_opts = { -- terminal settings for cmake, terminal will be used for run when `cmake_always_use_terminal` is false or true, will be used for all tasks except for debug when `cmake_always_use_terminal` is true
    ...
    -- ToggleTerm integration (requires toggleterm.nvim plugin)
    use_toggleterm = true, -- Use toggleterm if available, fallback to default terminal if not
    toggleterm_direction = "horizontal", -- toggleterm direction: "horizontal", "vertical", "tab", "float"
    toggleterm_close_on_exit = false, -- Whether to close toggleterm when process exits
    toggleterm_auto_scroll = true, -- Auto scroll to bottom when new output appears
  }
}
```
...
7. [how to](./docs/howto.md)
8. [toggleterm integration](./docs/toggleterm-integration.md)
