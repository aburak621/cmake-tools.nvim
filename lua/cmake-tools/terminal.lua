...
local has_terminal_api, terminal_api = pcall(require, "toggleterm.terminal")

local terminal = {
  id = nil,                  -- id for the unified terminal
  toggleterm_instance = nil, -- toggleterm Terminal instance
}

function terminal.has_active_job()
  if has_toggleterm and terminal.toggleterm_instance then
    -- For toggleterm, check if the terminal is running a job
    -- Check if terminal is open and has a valid job_id
    return terminal.toggleterm_instance:is_open() and
        terminal.toggleterm_instance.job_id and
        terminal.toggleterm_instance.job_id > 0
  elseif terminal.id then
    -- Fallback to default terminal behavior
    if not vim.api.nvim_buf_is_valid(terminal.id) then
      return false
    end

    local success, main_pid = pcall(vim.api.nvim_buf_get_var, terminal.id, "terminal_job_pid")
    if not success or not main_pid then
      return false
    end

    local success2, child_procs = pcall(vim.api.nvim_get_proc_children, main_pid)
    if not success2 or not child_procs then
      return false
    end
...
function terminal.show(terminal_opts)
  if has_toggleterm and terminal.toggleterm_instance then
    -- Use toggleterm to show the terminal
    terminal.toggleterm_instance:toggle()
    return
  end
...
-- Helper function to create toggleterm instance
function terminal.create_toggleterm_instance(term_name, opts)
  if not has_toggleterm or not has_terminal_api then
    log.warn("toggleterm is not installed. Falling back to default terminal.")
    return nil
  end

  -- Check if toggleterm is disabled in config
  if opts.use_toggleterm == false then
    return nil
  end

  local Terminal = terminal_api.Terminal
  local direction = opts.toggleterm_direction or opts.split_direction
  ...
  local size = opts.split_size or 15

  local term_instance = Terminal:new({
    display_name = term_name,
    direction = direction,
    size = size,
    close_on_exit = opts.toggleterm_close_on_exit or false,
    start_in_insert = opts.start_insert_in_launch_task or false,
    auto_scroll = opts.toggleterm_auto_scroll ~= false, -- default to true unless explicitly false
    float_opts = {
      border = "curved",
      winblend = 0,
    },
    -- Set up the terminal to work with cmake-tools
    on_create = function(term)
      -- Set filetype for cmake-tools terminal
      if term.bufnr and vim.api.nvim_buf_is_valid(term.bufnr) then
        vim.api.nvim_buf_set_option(term.bufnr, "filetype", "cmake_tools_terminal")
      end
    end,
    on_open = function(term)
      -- Terminal is ready to receive commands
    end,
  ...
  -- Don't open immediately - let the caller handle when to open
  -- This prevents issues with the terminal not being ready
...
function terminal.new_instance(term_name, opts)
  -- Try to create toggleterm instance first
  if has_toggleterm and has_terminal_api and opts.use_toggleterm ~= false then
    local success, term_instance = pcall(terminal.create_toggleterm_instance, term_name, opts)
    if success and term_instance and term_instance.bufnr then
      terminal.toggleterm_instance = term_instance
      terminal.id = term_instance.bufnr
      return term_instance.bufnr
    else
      log.warn("Failed to create ToggleTerm in new_instance: " .. tostring(term_instance or "unknown error"))
    end
  end
  ...
  vim.cmd(":" .. opts.split_direction .. " " .. opts.split_size .. "sp | :term") -- Create terminal in a split
  ...
  -- Renaming a terminal buffer creates a new hidden buffer, so duplicate terminals need to be deleted
  ...
  -- This is mainly for users to do filtering if necessary, as terminal does not have a default type.
  -- Example: using a filter in 'hardtime.nvim' to make sure
  -- we can use chained hjkl keys in succession in the terminal to scroll.
...
function terminal.send_data_to_terminal(buffer_idx, cmd, opts)
  -- Handle toggleterm instances
  if has_toggleterm and terminal.toggleterm_instance and terminal.toggleterm_instance.bufnr == buffer_idx then
    -- Show the terminal first if it's not already open
    if not terminal.toggleterm_instance:is_open() then
      terminal.toggleterm_instance:open()
    end

    -- Wait a moment for the terminal to fully initialize
    vim.defer_fn(function()
      -- Check if the terminal has a valid job_id before sending
      if terminal.toggleterm_instance.job_id and terminal.toggleterm_instance.job_id > 0 then
        ...
        local success, err = pcall(function()
          terminal.toggleterm_instance:send(formatted_cmd)
        end)

        if not success then
          log.error("Failed to send command to toggleterm: " .. (err or "unknown error"))
          -- Fallback: try to recreate the terminal
          terminal.toggleterm_instance = nil
          terminal.id = nil
        end
      else
        log.warn("ToggleTerm job_id is invalid, falling back to default terminal")
      ...
      if opts and not (opts.focus_on_launch_terminal or opts.focus_on_main_terminal) then
        vim.cmd("wincmd p") -- Goes back to previous window
  ...
  elseif osys.islinux then
    cmd = cmd .. " \n"
  elseif osys.iswsl then
    --NOTE: Technically, wsl-2 and linux are detected as linux. We might see a difference in wsl-1 vs wsl-2
    cmd = cmd .. " \n"
  end
...
function terminal.create_if_not_exists(term_name, opts)
  -- Check if we have a toggleterm instance already and it's valid
  if has_toggleterm and terminal.toggleterm_instance then
    -- Verify the instance is still valid
    if terminal.toggleterm_instance.bufnr and vim.api.nvim_buf_is_valid(terminal.toggleterm_instance.bufnr) then
      return true, terminal.toggleterm_instance.bufnr
    else
      -- Invalid instance, clear it
      terminal.toggleterm_instance = nil
      terminal.id = nil
    end
  end

  -- Try to create a new toggleterm instance
  if has_toggleterm and has_terminal_api and opts.use_toggleterm ~= false then
    local success, term_instance = pcall(terminal.create_toggleterm_instance, term_name, opts)
    if success and term_instance then
      terminal.toggleterm_instance = term_instance
      terminal.id = term_instance.bufnr
      return false, term_instance.bufnr
    else
      log.warn("Failed to create ToggleTerm instance: " .. tostring(term_instance or "unknown error"))
    end
  end
...
function terminal.stop()
  -- Handle toggleterm instances
  if has_toggleterm and terminal.toggleterm_instance then
    -- For toggleterm, we can close/kill the terminal
    if terminal.toggleterm_instance:is_open() then
      terminal.toggleterm_instance:close()
    end
    -- If we need to kill the job, we can send Ctrl+C
    if terminal.toggleterm_instance.job_id then
      terminal.toggleterm_instance:send("\x03") -- Send Ctrl+C
    end
    return
  end
